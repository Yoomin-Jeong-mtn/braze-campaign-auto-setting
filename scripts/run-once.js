...
(async () => {
  ...
  for (const campaign of campaigns) {
    ...
    if (!templateCampaignId) {
      ...
      const errorReason = `세그먼트 "${campaign.segment}" 템플릿 없음`;
      await updateCampaignResult(campaign.rowIndex, { status: '세팅 실패', errorReason });
      results.push({ ...campaign, status: '세팅 실패', errorReason });
      continue;
    }
    ...
    try {
      ...
      results.push({ ...campaign, status: '세팅 완료', brazeUrl });
      ...
      results.push({ ...campaign, status: '세팅 실패', errorReason: err.message });
    }
  }
